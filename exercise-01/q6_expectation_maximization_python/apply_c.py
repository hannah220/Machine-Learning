...
from estGaussMixEM_k import estGaussMixEM_k
...
for idx in range(3):
    ...
    print('means is:\n', means)
    print('testparams[0, idx] means"\n', testparams[0, idx])

    print('weights is:\n', weights)
    print('testparams[1, idx] weights"\n', testparams[1, idx])

    print('covariances is:\n', covariances)
    print('testparams[2, idx] covariances"\n', testparams[2, idx])

    
    absmeandiff = abs(means - testparams[0, idx])
    absweightdiff = abs(np.matrix(weights) - np.matrix(testparams[1, idx]))
...
for idx in range(3):
    ...
    print('covariance:\n', covariance)
    print('regularized covariance[:, :, idx]:\n', regularized_cov[:, :, idx])
...
#for k in range(num):
#    # compute GMM
#    weights, means, covariances = estGaussMixEM_k(data[2], k+1, n_iter, epsilon)
#    logLikelihood[k] = getLogLikelihood_k(means, weights, covariances, data[2], k+1)
#
## plot result
#plt.subplot()
